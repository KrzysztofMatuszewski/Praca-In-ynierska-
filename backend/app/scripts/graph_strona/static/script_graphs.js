document.addEventListener('DOMContentLoaded', function() {
    ...
    // Second Chart: Loss Graph
    var epochs = Array.from({length: loss.length}, (_, i) => i + 1);

    var ctxLoss = document.getElementById('lossGraph').getContext('2d');
    var lossGraph = new Chart(ctxLoss, {
        type: 'line',
        data: {
            labels: epochs,
            datasets: [
    ...
    var epochs = Array.from({length: loss.length}, (_, i) => i + 1); // Creates [1, 2, 3, ..., n] for n epochs

    var ctx = document.getElementById('accuracyGraph').getContext('2d');
    var accuracyGraph = new Chart(ctx, {
        type: 'line',
        data: {
            labels: epochs,
            datasets: [
    ...
    function createFeatureData(data, anomaly_array) {
        const booleanAnomalyArray = anomaly_array.map(value => value === 'True');
    ...
    function plotFeatureDistributions() {
        ...
        filtered_headers.forEach((feature_name, index) => {
            const { uniqueValues, normalCounts, anomalyCounts } = createFeatureData(column_arrays[feature_name], anomaly_array);
