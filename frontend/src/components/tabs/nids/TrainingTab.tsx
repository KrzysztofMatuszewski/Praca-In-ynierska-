...
import { LossDataItem } from '@/types/nids';
import { prepareMultiSeriesLossData } from '@/utils/dataTransformers';

interface TrainingTabProps {
  nidsLossData: { [folder: string]: LossDataItem[] };
  loading: boolean;
}
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-64">
            {Object.keys(nidsLossData).length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={prepareMultiSeriesLossData(nidsLossData)}>
                  ...
                  {Object.keys(nidsLossData).map((folder, index) => (
                    <Line 
                      key={`${folder}_loss`}
                      type="monotone" 
